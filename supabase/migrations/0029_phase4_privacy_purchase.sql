-- Fase 4 (corrección) — Privacidad de saldos + restricción de solicitud de compra.
--  (1) El snapshot NO expone los saldos ajenos (ni siquiera al anfitrión, que también juega). Cada
--      jugador solo ve el suyo; los movimientos (ledger) siguen mostrando importes para el control.
--  (2) Solo se puede SOLICITAR comprar la propiedad en cuya casilla estás, en tu turno, activo, en
--      curso, disponible y sin subasta activa. Validado en backend (no se confía en la UI).

-- ── 1) request_property_purchase: requiere jugador actual + estar en esa casilla ──
...
declare rt public.game_runtime; me public.players; c public.property_catalog; v_idem jsonb;
        r public.property_purchase_requests; v_existing public.property_purchase_requests; v_here text;
begin
  ...
  me := public._require_active_player(p_game);                                       -- activo y no espectador
  if me.public_ref <> rt.turn_order_refs[rt.turn_index] then raise exception 'NOT_CURRENT_PLAYER'; end if;
  ...
  -- Debes estar EN la casilla de esa propiedad (posición actual -> casilla -> property_ref).
  perform public._p4_ensure_positions(p_game);
  select bs.property_ref into v_here
    from public.player_positions pp
    join public.board_spaces bs on bs.board_key = pp.board_key and bs.space_index = pp.space_index and bs.active
   where pp.game_id = p_game and pp.player_ref = me.public_ref;
  if v_here is distinct from p_property_ref then raise exception 'NOT_ON_PROPERTY'; end if;
...
-- ── 2) Snapshot: saldos ajenos ocultos + boards/spaces con marca 'provisional' ─────
...
declare g public.games; rt public.game_runtime; me public.players; v_cur text; v_players jsonb; v_ledger jsonb;
        ...
        v_start_bonus int; v_boards jsonb; v_spaces jsonb; v_positions jsonb; v_my_pos jsonb; v_current jsonb;
...
begin
  ...
  -- Privacidad de saldos: cada jugador solo ve el SUYO (ni el anfitrión ve los ajenos).
