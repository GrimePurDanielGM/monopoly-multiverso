-- Fase 4 (corrección 2) — Snapshot: expone guardianes por casilla y los enlaces de montaje entre
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'provisional', s.provisional, 'guardian', s.guardian, 'links_to_board', s.links_to_board)
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'links_to_board', s.links_to_board) order by s.board_key), '[]'::jsonb)
    into v_links from public.board_spaces s where s.active and s.guardian;
