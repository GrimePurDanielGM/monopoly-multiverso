-- Fase 4 — Snapshot activo ampliado con tablero y posiciones (todo saneado, sin ids internos):
--   boards (anillo + bonus), spaces (catálogo de casillas), positions (ficha de cada jugador),
--   my_position, current_space (casilla del jugador local) y last_roll / last_move.
...
begin
  ...
  -- ── Fase 4: tablero, casillas y posiciones (saneado) ────────────────────────────
  select coalesce(jsonb_agg(jsonb_build_object(
           'board_key', t.board_key, 'ring_size', t.n, 'start_bonus', v_start_bonus) order by t.board_key), '[]'::jsonb)
    into v_boards
    from (select board_key, count(*)::int as n from public.board_spaces where active group by board_key) t;
  ...
  -- Fichas de los jugadores mostrados (activos + en bancarrota), si tienen posición.
  select coalesce(jsonb_agg(jsonb_build_object(
