-- Fase 1 — Resolución segura code -> snapshot para la carga fría de /sala/:code.
-- Sin SELECT directo a games (revocado en 0008) y sin exponer game_id por una RPC aparte.
-- Patrón: helper interno ÚNICO (_lobby_snapshot) que recibe el caller explícitamente y
-- construye el JSON saneado; las dos RPC públicas (por id y por código) delegan en él.
-- No se duplica el bloque JSON ni se depende del auth.uid() implícito dentro del constructor.
-- No modifica peek_code/peek_game, RLS, PBKDF2 ni migraciones previas.

-- ---------- Helper interno (NO se concede a authenticated; solo lo usan las RPC públicas) ----------
...
begin
  ...
  -- Exige jugador ACTIVO y no expulsado (el auth_uid solo se usa internamente, jamás se devuelve).
  select * into me from public.players where game_id = p_game and auth_uid = p_caller and kicked_at is null;
...
-- ---------- RPC por id: ahora delega en el helper (mismo contrato que antes) ----------
create or replace function public.get_lobby_snapshot(p_game uuid)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
begin
  if auth.uid() is null then raise exception 'NOT_AUTHENTICATED'; end if;
  return public._lobby_snapshot(p_game, auth.uid());
end $$;

-- ---------- RPC por código: normaliza, resuelve y delega (carga fría de /sala/:code) ----------
...
-- ---------- Grants ----------
revoke execute on function public._lobby_snapshot(uuid, uuid)          from public;  -- helper interno
revoke execute on function public.get_lobby_snapshot(uuid)             from public;
grant  execute on function public.get_lobby_snapshot(uuid)             to authenticated;  -- reafirma (de 0008)
