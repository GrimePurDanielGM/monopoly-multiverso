-- ============================================================================
-- Fase 7 — Tratos: flujos principales (crear/aceptar/ejecutar, host, contraoferta, carta, acuerdo).
...
-- T1) dinero↔dinero: p1 ofrece 300, no requiere anfitrión, queda pending.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p1u text:=pg_temp._ctx('p1_uid'); p2 text:=pg_temp._ctx('p2'); r jsonb; begin
  perform pg_temp._as_user(p1u); r := create_trade_proposal(gid, p2, 300, 0, null, null, null, null, null, gen_random_uuid());
  insert into _ctx values ('t1', r->>'trade_ref') on conflict (k) do update set v=excluded.v; perform pg_temp._as_admin();
  perform pg_temp._rec('T1) dinero↔dinero: pending sin anfitrión', (r->>'status')='pending' and (r->>'requires_host')='false' and pg_temp._tstatus(gid, r->>'trade_ref')='pending');
end $$;
-- T2) p2 acepta → executed; saldos movidos.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p2u text:=pg_temp._ctx('p2_uid'); p1 text:=pg_temp._ctx('p1'); p2 text:=pg_temp._ctx('p2'); t text:=pg_temp._ctx('t1'); b1 bigint; b2 bigint; r jsonb; begin
  perform pg_temp._as_admin(); b1:=pg_temp._bal(gid,p1); b2:=pg_temp._bal(gid,p2);
  perform pg_temp._as_user(p2u); r := accept_trade_proposal(t, pg_temp._ver(gid), gen_random_uuid()); perform pg_temp._as_admin();
  perform pg_temp._rec('T2) aceptar dinero↔dinero ejecuta y mueve saldos', (r->>'status')='executed' and pg_temp._bal(gid,p1)=b1-300 and pg_temp._bal(gid,p2)=b2+300);
end $$;
-- T3) propiedad↔dinero requiere anfitrión.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p1u text:=pg_temp._ctx('p1_uid'); p2 text:=pg_temp._ctx('p2'); r jsonb; begin
  perform pg_temp._as_user(p1u); r := create_trade_proposal(gid, p2, 0, 500, array['cl-ronda-valencia'], null, null, null, null, gen_random_uuid());
  insert into _ctx values ('t3', r->>'trade_ref') on conflict (k) do update set v=excluded.v; perform pg_temp._as_admin();
  perform pg_temp._rec('T3) propiedad↔dinero requiere anfitrión', (r->>'requires_host')='true' and pg_temp._tstatus(gid, r->>'trade_ref')='pending');
end $$;
-- T4) p2 acepta → host_review (no ejecuta aún).
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p2u text:=pg_temp._ctx('p2_uid'); t text:=pg_temp._ctx('t3'); r jsonb; begin
  perform pg_temp._as_user(p2u); r := accept_trade_proposal(t, pg_temp._ver(gid), gen_random_uuid()); perform pg_temp._as_admin();
  perform pg_temp._rec('T4) aceptar con propiedad → host_review', (r->>'status')='host_review' and pg_temp._ownerof(gid,'cl-ronda-valencia')=pg_temp._ctx('p1'));
end $$;
-- T5) host aprueba → executed; Ronda pasa a p2, 500 p2→p1.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; host text:=pg_temp._ctx('host_uid'); p1 text:=pg_temp._ctx('p1'); p2 text:=pg_temp._ctx('p2'); t text:=pg_temp._ctx('t3'); b1 bigint; b2 bigint; r jsonb; begin
  perform pg_temp._as_admin(); b1:=pg_temp._bal(gid,p1); b2:=pg_temp._bal(gid,p2);
  perform pg_temp._as_user(host); r := resolve_trade_proposal(t, true, pg_temp._ver(gid)); perform pg_temp._as_admin();
  perform pg_temp._rec('T5) host aprueba: propiedad y dinero cambian', (r->>'status')='executed' and pg_temp._ownerof(gid,'cl-ronda-valencia')=p2 and pg_temp._bal(gid,p2)=b2-500 and pg_temp._bal(gid,p1)=b1+500);
end $$;
-- T6) contraoferta vuelve a la otra parte y se ejecuta.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p1u text:=pg_temp._ctx('p1_uid'); p2u text:=pg_temp._ctx('p2_uid'); p1 text:=pg_temp._ctx('p1'); r jsonb; t text; s1 text; s2 text; begin
  perform pg_temp._as_user(p2u); r := create_trade_proposal(gid, p1, 100, 0, null, null, null, null, null, gen_random_uuid()); t:=r->>'trade_ref';
  perform pg_temp._as_user(p1u); r := counter_trade_proposal(t, 0, 50, null, null, null, null, null, gen_random_uuid()); s1:=r->>'status';
  perform pg_temp._as_user(p2u); r := accept_trade_proposal(t, pg_temp._ver(gid), gen_random_uuid()); s2:=r->>'status'; perform pg_temp._as_admin();
  perform pg_temp._rec('T6) contraoferta vuelve al otro y luego se ejecuta', s1='countered' and s2='executed');
end $$;
-- T7) carta conservable se transfiere al aprobar.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p1u text:=pg_temp._ctx('p1_uid'); p2u text:=pg_temp._ctx('p2_uid'); host text:=pg_temp._ctx('host_uid'); p1 text:=pg_temp._ctx('p1'); p2 text:=pg_temp._ctx('p2'); r jsonb; t text; c0 int; begin
  perform pg_temp._as_admin(); perform pg_temp._givecard(gid, p1, 'chance-jail-free'); c0:=pg_temp._cardcount(gid,p2);
  perform pg_temp._as_user(p1u); r := create_trade_proposal(gid, p2, 0, 0, null, null, array['chance-jail-free'], null, null, gen_random_uuid()); t:=r->>'trade_ref';
  perform pg_temp._as_user(p2u); perform accept_trade_proposal(t, pg_temp._ver(gid), gen_random_uuid());
  perform pg_temp._as_user(host); r := resolve_trade_proposal(t, true, pg_temp._ver(gid)); perform pg_temp._as_admin();
  perform pg_temp._rec('T7) carta conservable se transfiere al aprobar', (r->>'status')='executed' and pg_temp._cardcount(gid,p2)=c0+1 and exists(select 1 from public.game_held_cards where game_id=gid and player_ref=p2 and card_ref='chance-jail-free'));
end $$;
-- T8) acuerdo personal registrado + requiere anfitrión.
do $$ declare gid uuid:=pg_temp._ctx('gid')::uuid; p1u text:=pg_temp._ctx('p1_uid'); p2 text:=pg_temp._ctx('p2'); r jsonb; t text; v_agree text; begin
  perform pg_temp._as_user(p1u); r := create_trade_proposal(gid, p2, 0, 0, null, null, null, null, 'No te cobro alquiler 1 turno', gen_random_uuid()); t:=r->>'trade_ref';
  perform pg_temp._as_admin(); select agreement_text into v_agree from public.game_trade_proposals where game_id=gid and public_ref=t;
  perform pg_temp._rec('T8) acuerdo personal registrado + requiere anfitrión', v_agree='No te cobro alquiler 1 turno' and (r->>'requires_host')='true');
end $$;
