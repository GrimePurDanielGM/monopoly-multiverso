...
vi.mock('../lib/api', () => ({
  ...
  movePlayer: () => Promise.resolve({ ok: true, data: true }),
  rollAndMove: () => Promise.resolve({ ok: true, data: true }),
  moveWithPhysicalRoll: () => Promise.resolve({ ok: true, data: true }),
  hostSetPlayerPosition: () => Promise.resolve({ ok: true, data: true }),
  resolveJunction: () => Promise.resolve({ ok: true, data: true }),
  payRent: () => Promise.resolve({ ok: true, data: true }),
  payUtilityRent: () => Promise.resolve({ ok: true, data: true }),
  setDiceMode: () => Promise.resolve({ ok: true, data: true }),
  payJailRelease: () => Promise.resolve({ ok: true, data: true }),
  redeemJailCard: () => Promise.resolve({ ok: true, data: true }),
  resolveCard: () => Promise.resolve({ ok: true, data: true }),
  payPending: () => Promise.resolve({ ok: true, data: true }),
  requestPropertyPurchase: () => Promise.resolve({ ok: true, data: true }),
  resolvePropertyPurchase: () => Promise.resolve({ ok: true, data: true }),
  startPropertyAuction: () => Promise.resolve({ ok: true, data: true }),
  placePropertyBid: () => Promise.resolve({ ok: true, data: true }),
  closePropertyAuction: () => Promise.resolve({ ok: true, data: true }),
  cancelPropertyAuction: () => Promise.resolve({ ok: true, data: true }),
  requestBankruptcy: () => Promise.resolve({ ok: true, data: true }),
  resolveBankruptcy: () => Promise.resolve({ ok: true, data: true }),
  requestLeaveActive: () => Promise.resolve({ ok: true, data: true }),
  resolveLeaveActive: () => Promise.resolve({ ok: true, data: true }),
  removeActivePlayer: () => Promise.resolve({ ok: true, data: true }),
  pauseGame: () => Promise.resolve({ ok: true, data: true }),
  ...
  it('renderiza la pantalla de partida activa en estado active', async () => {
    ...
    activeMock.mockResolvedValue({
      ...
      data: {
        ...
        players: [{ public_ref: 'P-1', display_name: 'Anfitrión', token_id: 'delorean', balance: 3000, is_current: true, status: 'active' }],
        ledger_recent: [],
        properties: [],
        auctions: [],
        purchase_requests: [],
