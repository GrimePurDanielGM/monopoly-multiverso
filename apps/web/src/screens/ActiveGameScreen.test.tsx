...
function snap(over: Partial<ActiveSnapshot> = {}): ActiveSnapshot {
  return {
    game: { code: 'ABC234', status: 'active', config: { initial_money: 3000, min_players: 6, max_players: 16, allow_late_join: false, start_bonus: 200, dice_mode: 'virtual_only', initial_houses_available: 32, initial_hotels_available: 12, allow_build_without_monopoly: false, allow_trade_built_properties: false, parking_mode: 'pot' } },
...
describe('ActiveGameScreen — control y estados', () => {
  it('en pausa muestra el aviso y deshabilita acciones', () => {
    renderScreen(snap({ runtime_status: 'paused', current_landing_rent_resolved: false, building_stock: { houses_available: 32, hotels_available: 12 }, building_requests: [], my_building_requests: [], incoming_trades: [], outgoing_trades: [], trade_reviews: [], recent_trades: [], my_card_transfers: [], control: { paused_by_ref: 'P-1', finished_by_ref: null, reason: 'café' } }));
  ...
  it('finalizada muestra la pantalla final sin acciones', () => {
    renderScreen(snap({ runtime_status: 'finished', current_landing_rent_resolved: false, building_stock: { houses_available: 32, hotels_available: 12 }, building_requests: [], my_building_requests: [], incoming_trades: [], outgoing_trades: [], trade_reviews: [], recent_trades: [], my_card_transfers: [], control: { paused_by_ref: null, finished_by_ref: 'P-1', reason: null } }));
...
describe('ActiveGameScreen — propiedades', () => {
  ...
  it('pagar alquiler abre confirmación; confirmar llama payRent una vez', async () => {
    // me = P-1 (host); la propiedad es de P-2. rent_due (alquiler avanzado) ≠ base_rent: el diálogo muestra rent_due.
    renderScreen(snap({ properties: [PROP({ owner_ref: 'P-2', base_rent: 25, rent_due: 120 })] }));
    fireEvent.click(screen.getByRole('button', { name: 'Ver tablero de propiedades' }));
    fireEvent.click(screen.getByRole('button', { name: 'Pagar alquiler' }));
    const dlg = screen.getByRole('dialog', { name: 'Pagar alquiler' });
    expect(dlg).toBeInTheDocument();
    expect(within(dlg).getByText(/120/)).toBeInTheDocument(); // muestra el alquiler calculado, no el base (25)
