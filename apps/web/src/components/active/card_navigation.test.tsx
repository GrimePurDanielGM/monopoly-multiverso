...
describe('PropertyCardModal — construir sin monopolio (item 2)', () => {
  ...
  it('con la regla ACTIVADA, sin monopolio SÍ ofrece solicitar construir', () => {
    const s = snap([mineNoMono], { game: { ...snap([mineNoMono]).game, config: { ...snap([mineNoMono]).game.config, allow_build_without_monopoly: true, allow_trade_built_properties: false, parking_mode: 'pot', start_invest_pct: 0 } } });
