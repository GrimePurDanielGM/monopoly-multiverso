...
/** Marco común: cabecera, avisos (offline / sin configurar), PWA y contenido. Mobile-first;
 *  se ensancha en escritorio (las pantallas estrechas se auto-centran a max-w-md). */
export function AppShell() {
  const online = useConnectionStore((s) => s.online);
  const status = useSessionStore((s) => s.status);

  return (
    <div className="mx-auto flex min-h-full w-full max-w-md flex-col bg-slate-900 text-slate-100 lg:max-w-5xl">
